...
const Query = {
    ...
    getBetList: async (
        _,
        { page = 1, pageSize = 10, filters = {} }: { page: number; pageSize: number; filters: Filters }
    ): Promise<any> => {
        if (page < 1) page = 1
        ...
        const filter: WhereOptions<Filters> = {}
        if (filters.userId) {
    ...
    getBestBetPerUser: async (_, { page = 1, pageSize = 10 }) => {
        ...
        const filteredBets = Object.values(
            rows.reduce((acc, currentValue) => {
                if (!acc[currentValue.userId] && currentValue.win) {
...
const Mutation = {
    createBet: async (_, { quantity = 1, clientSeed, testSpin, betAmount, chance }, { user }) => {
        if (quantity < 1 || quantity > 5) throw new Error('Invalid params.')
        if (betAmount <= 0) throw new Error('Invalid betAmount.')
        if (chance > 100 || chance <= 0) throw new Error('Invalid chance.')
        ...
        if (totalCost > updatedUser.balance) throw new Error('Not enough balance to complete this request.')

        let updated = await updateUserBalanceSafe(id, -totalCost)

        if (!updated) throw new Error('Please wait before placing another bet.')
