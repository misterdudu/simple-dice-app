 ...
 */
export async function diceBet({ userId, quantity, betAmount, chance, userServerSeed, userClientSeed, testSpin }) {
    try {
        const bets = [];
        const rollPF = [];
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            let bet;
            if (!testSpin)
                bet = await Bet.create({ userId, betAmount, chance, payout: finalPayout, win });
